...
async function createManualOrder(order) {
  ...
  await db.execute(
    `INSERT INTO minecraft_shop_orders
      (id, payment_intent_id, player, product_id, product_name, amount, currency, commands_json, status, message)
     VALUES (?, NULL, ?, ?, ?, 0, 'eur', ?, 'pending_delivery', ?)`,
    [
      order.id,
      order.player,
      order.productId,
      order.productName,
      JSON.stringify(order.commands || []),
      order.message || 'Handmatig cadeau aangemaakt in-game'
    ]
  );
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (result.affectedRows === 1) {
        claimed.push({ ...mapRow(row), status: 'processing' });
      }
    }
...
async function retryOrder(orderId) {
  ...
  const [result] = await db.execute(
    `UPDATE minecraft_shop_orders
     SET status = 'pending_delivery', claimed_at = NULL, message = 'Handmatig opnieuw aangeboden', updated_at = NOW()
...
async function listOrders(limit = 45) {
  await ensureSchema();
  const db = getPool();
  const safeLimit = Math.min(Math.max(Number(limit) || 45, 1), 100);
